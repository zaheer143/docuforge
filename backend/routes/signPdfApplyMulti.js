...
// ✅ Supports PNG dataURL (your frontend uses PNG from canvas/typed)
...
router.post("/apply-multi", upload.single("pdf"), async (req, res) => {
  let pdfPath = "";
  try {
    ...
    // Legacy payload (optional): signaturesJson = { signerId: dataUrl }
    const signaturesMap = parseJson(req.body?.signaturesJson || "{}", {});

    // ✅ Your current frontend payload: signers = [{ id, signatureDataUrl, ... }]
    const signersArr = parseJson(req.body?.signers || "[]", []);

    const font = await pdfDoc.embedFont(StandardFonts.Helvetica);
    const fontBold = await pdfDoc.embedFont(StandardFonts.HelveticaBold);

    // Map signerId -> embedded PNG image (supports both payload formats)
    const signerIdToImage = new Map();

    // 1) signaturesJson map
    ...
    // 2) signers array (the one your frontend actually sends)
    if (Array.isArray(signersArr) && signersArr.length) {
      for (const s of signersArr) {
        const signerId = s?.id != null ? String(s.id) : "";
        const bytes = pngBytesFromDataUrl(s?.signatureDataUrl);
        if (!signerId || !bytes) continue;
        try {
          const img = await pdfDoc.embedPng(bytes);
          signerIdToImage.set(signerId, img);
        } catch {
          // ignore
        }
      }
    ...
    if (Array.isArray(placements) && placements.length > 0) {
      for (const p of placements) {
        ...
        if (img) {
          ...
          // fallback box + debug text (kept but improved)
          page.drawRectangle({
            x,
            y,
            width: w,
          ...
          page.drawText(signerId ? `SIGN (${signerId})` : "SIGN", {
            x: x + 6,
            y: y + h / 2 - 6,
            size: 12,
            font: fontBold,
